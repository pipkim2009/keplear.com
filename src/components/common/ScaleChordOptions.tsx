...
const ScaleChordOptions: React.FC<ScaleChordOptionsProps> = ({
  ...
  const [keyboardSelectedScale, setKeyboardSelectedScale] = useState<KeyboardScale>(
    KEYBOARD_SCALES[0]
  )
  ...
  const [keyboardSelectedChord, setKeyboardSelectedChord] = useState<KeyboardChord>(
    KEYBOARD_CHORDS[0]
  )
  ...
  const playDiagramSound = useCallback(async () => {
    ...
    const playFn =
      instrument === 'guitar' ? playGuitarNote : instrument === 'bass' ? playBassNote : playNote
      ...
      if (instrument === 'guitar') {
        ...
        if (isScaleMode) {
          ...
          if (
            ...
            selectedBoxIndex < availableBoxes.length
          ) {
          ...
          if (
            showChordPositions &&
            availableChordBoxes.length > 0 &&
            selectedChordBoxIndex < availableChordBoxes.length
          ) {
        ...
        if (isScaleMode) {
          ...
          if (
            showPositions &&
            availableBassBoxes.length > 0 &&
            selectedBoxIndex < availableBassBoxes.length
          ) {
            positions = applyScaleBoxToBass(availableBassBoxes[selectedBoxIndex])
          } else {
            positions = applyScaleToBass(selectedRoot, selectedBassScale, bassNotes)
          }
          // Find root positions
          positions.forEach(pos => {
          ...
          if (
            showChordPositions &&
            availableBassChordBoxes.length > 0 &&
            selectedChordBoxIndex < availableBassChordBoxes.length
          ) {
    ...
    if (instrument === 'guitar') {
      ...
      if (
        ...
        availableBoxes.forEach(box => {
          ...
          const isAlreadyApplied = appliedScales.some(
            s =>
              s.displayName.includes(boxName) ||
              s.displayName.includes(`Frets ${box.minFret}-${box.maxFret}`)
          )
          if (!isAlreadyApplied) {
      ...
      if (
        ...
        availableBassBoxes.forEach(box => {
          ...
          if (!isAlreadyApplied) {
            onScaleBoxSelect(box as ScaleBox)
          }
        })
      } else if (
        showPositions &&
        ...
        onScaleBoxSelect(availableBassBoxes[selectedBoxIndex] as unknown as ScaleBox)
      } else if (onScaleSelect) {
        onScaleSelect(selectedRoot, selectedBassScale as unknown as GuitarScale)
      }
      ...
      if (
        ...
        availableChordBoxes.forEach((chordBox, index) => {
          ...
          if (!isAlreadyApplied) {
            ...
            onChordShapeSelect(chordShapeFromBox as unknown as ChordShape)
          }
        })
      } else if (
        showChordPositions &&
      ...
      if (
        ...
        availableBassChordBoxes.forEach((chordBox, index) => {
          ...
          if (!isAlreadyApplied) {
            ...
            onChordShapeSelect(bassChordShapeFromBox as unknown as ChordShape)
          }
        })
      } else if (
        showChordPositions &&
        ...
        onChordSelect(selectedChordRoot, selectedBassChord as unknown as GuitarChord)
      }
  ...
  return (
    <div
      ref={containerRef}
      className={`scale-options-container ${isExpanded ? 'expanded' : 'collapsed'}`}
    >
      <button
      ...
      {isExpanded && (
        ...
        >
          <div
            className={`scale-options-content ${showOnlyAppliedList ? ((appliedChords.length > 0 && appliedScales.length === 0) || (appliedChords.length > 0 && appliedScales.length > 0 && !isScaleMode) ? 'chord-mode' : '') : !isScaleMode ? 'chord-mode' : ''}`}
          >
            {showOnlyAppliedList ? (
              <>
                  ...
                  <div className="control-section scale-theme">
                    ...
                    <div className="applied-scales-list">
                      {appliedScales.map(appliedScale => (
                        <div
                          key={appliedScale.id}
                          className={`applied-scale-item instrument-${instrument}`}
                        >
                  ...
                  <div className="control-section chord-theme">
                    ...
                    <div className="applied-chords-list">
                      {appliedChords.map(appliedChord => (
                        <div
                          key={appliedChord.id}
                          className={`applied-chord-item instrument-${instrument}`}
                        >
                          <div className="applied-item-info">
                        ...
                        <select
                          value={
                            instrument === 'guitar'
                              ? selectedScale.name
                              : instrument === 'bass'
                                ? selectedBassScale.name
                                : keyboardSelectedScale.name
                          }
                          onChange={e => {
                            if (instrument === 'guitar') {
                        ...
                        <select
                          value={
                            instrument === 'guitar'
                              ? selectedChord.name
                              : instrument === 'bass'
                                ? selectedBassChord.name
                                : keyboardSelectedChord.name
                          }
                          onChange={e => {
                            if (instrument === 'guitar') {
            ...
            <div className="learn-diagram-modal" onClick={e => e.stopPropagation()}>
              <div className="learn-diagram-header">
                ...
                <div className="learn-diagram-actions">
                  ...
                  <button className="learn-diagram-close" onClick={() => setLearnDiagramData(null)}>
                    ×
                  </button>
                </div>
              </div>
                    ...
                    {instrument !== 'keyboard' && (
                      <div className="diagram-info">
                        {(learnDiagramData.item as AppliedScale).scale.intervals && (
                          <p>
                            {(learnDiagramData.item as AppliedScale).scale.intervals.join(' - ')}
                          </p>
                        )}
                      </div>
                    )}
                    ...
                    {instrument !== 'keyboard' && (
                      <div className="diagram-info">
                        {(learnDiagramData.item as AppliedChord).chord.intervals && (
                          <p>
                            {(learnDiagramData.item as AppliedChord).chord.intervals.join(' - ')}
                          </p>
                        )}
                      </div>
                    )}
