...
const Guitar: React.FC<GuitarProps> = ({
  ...
  const [currentScale, setCurrentScale] = useState<{ root: string; scale: GuitarScale } | null>(
    null
  )
  ...
  const [currentChord, setCurrentChord] = useState<{ root: string; chord: GuitarChord } | null>(
    null
  )
    ...
    if (onNoteClick) {
      ...
      if (noteName) {
        ...
        const guitarNote = guitarNotes.find(
          note => note.string === guitarString && note.fret === fretIndex + 1
        )
    ...
    for (let stringIndex = 0; stringIndex < 6; stringIndex++) {
      if (isOpenStringSelected(stringIndex)) {
        ...
        if (openNote) {
          ...
          melodyNotes.push({
            id: `g-s${guitarString}-f0`, // Include ID for open string
            name: openNote.name,
            frequency: openNote.frequency,
            isBlack: openNote.name.includes('#'),
      ...
      for (let fretIndex = 0; fretIndex < 24; fretIndex++) {
        if (isNoteSelected(stringIndex, fretIndex)) {
          ...
          if (noteName) {
            ...
            melodyNotes.push({
              id: `g-s${guitarString}-f${fretIndex + 1}`, // Include ID for fretted note
              name: noteName,
  ...
  const handleSetManualNotes = useCallback((noteIds: string[]) => {
    ...
    noteIds.forEach(noteId => {
      // Parse note ID format: "g-s{string}-f{fret}" e.g., "g-s1-f3"
      const match = noteId.match(/^g-s(\d+)-f(\d+)$/)
      if (match) {
        const guitarString = parseInt(match[1]) // 1-6
        const fret = parseInt(match[2]) // 0-24

        // Convert guitar string number to visual stringIndex
        // stringMapping[stringIndex] = guitarString, so stringIndex = 6 - guitarString
      ...
      <div className={`fretboard ${showNotes ? 'melody-active' : ''}`}>
        ...
        {!disableNoteSelection && (
          <div className="fret-checkbox-container" style={{ left: '4.5px', bottom: '-40px' }}>
            ...
            />
            <label htmlFor="fret-open" className="fret-checkbox-label">
              0
            </label>
          </div>
        )}
            ...
            {!disableNoteSelection && (
              <div className="fret-checkbox-container">
                <input
                  ...
                  onMouseLeave={() => setHoveredFret(null)}
                />
                <label htmlFor={`fret-${index + 1}`} className="fret-checkbox-label">
                  {index + 1}
                </label>
              </div>
            )}
        ...
        {!disableNoteSelection &&
          [...Array(6)].map((_, index) => (
            ...
            >
              ...
              />
              <label htmlFor={`string-${index}`} className="string-checkbox-label">
                {6 - index}
              </label>
            </div>
          ))}
        ...
        {[...Array(6)].map((_, stringIndex) => {
          ...
          const noteKey = `${stringIndex}-open`
          const noteId = `g-s${stringIndex + 1}-f0` // Guitar note ID format for open string
          ...
          return (
            ...
            >
              <span className="note-name">{openNote.name}</span>
            </div>
          )
        })}
        ...
        {[...Array(6)].map((_, stringIndex) =>
          [...Array(24)].map((_, fretIndex) => {
            ...
            // For chords, match by chord ID to get specific shape; otherwise check note IDs then names
            const noteKey = `${stringIndex}-${fretIndex}`
            const noteId = `g-s${stringIndex + 1}-f${fretIndex}` // Guitar note ID format
            let isCurrentlyPlaying = false
            if (showNotes) {
              // First priority: check by note ID for position-accurate matching
              if (currentlyPlayingNoteIds.length > 0 && currentlyPlayingNoteIds.includes(noteId)) {
            ...
            return (
              ...
              >
                <span className="note-name">{getNoteName(noteName)}</span>
              </div>
            )
          })
        )}
        ...
        {previewPositions?.positions?.map((pos, idx) => {
          ...
          const leftPosition =
            pos.fretIndex === 0 ? -2.5 : pos.fretIndex === 1 ? 27 : pos.fretIndex * 54 - 34
